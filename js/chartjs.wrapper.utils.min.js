...
var createGradient=this.createGradient;this.checkContext(document,"flt-platform-view").then(e=>{var platformViews=document.querySelectorAll("flt-platform-view");for(let i=0;i<platformViews.length;i++){const platformView=platformViews[i];this.checkContext(platformView.shadowRoot,"#"+chartId).then(chart=>{console.log('chartFinded! ---- ',chartId,chart);var _chartContext=chart.getContext("2d");config.data.datasets.forEach(element=>{if(element.backgroundColor){element.backgroundColor=createGradient(element.backgroundColor,_chartContext)}
