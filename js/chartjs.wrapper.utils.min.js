...
window.chartJSWrapperPlugin=class checkContext{async init(element,selector){while(element.querySelector(selector)===null){await new Promise(resolve=>setTimeout(resolve,500))}
...
var createGradient=this.createGradient;var init=this.init;init(document,"flt-platform-view").then(e=>{var platformViews=document.querySelectorAll("flt-platform-view");for(let i=0;i<platformViews.length;i++){const platformView=platformViews[i];init(platformView.shadowRoot,"#"+chartId).then(chart=>{var _chartContext=chart.getContext("2d");config.data.datasets.forEach(element=>{if(element.backgroundColor){element.backgroundColor=createGradient(element.backgroundColor,_chartContext)}
